...
random_device rd;
mt19937 rng(rd());
uniform_int_distribution<std::mt19937::result_type> random100(0,10);
...
void multMatrixWithThreads(int n, int countThreads, vector<vector<int>> &matrix1, vector<vector<int>> &matrix2, vector<vector<int>> matrixRes){
    ...
    for(int i = 0; i < countThreads; i++){
        if (n <= i * val) break;
        cout<<i<<endl;
        // calculateForThread(val * i , (n - val * (i + 1)  >= 0? val * (i + 1) : val * i + (n % (val * i))), matrix1, matrix2, matrixRes);
        threads.push_back(std::thread(&calculateForThread, val * i , (n - val * (i + 1)  >= 0? val * (i + 1) : val * i + (n % (val * i))), matrix1, matrix2, matrixRes));
        //  cout << i << " thread(calculateForThread, " << val * i << ", " << (n - val * (i + 1)  >= 0? val * (i + 1) : val * i + (n % (val * i)))  << ")" << endl;
    }

    // for(int i = 0; i < threads.size(); i++){
    //     // cout << i << " " << threads[i].joinable() << endl;
